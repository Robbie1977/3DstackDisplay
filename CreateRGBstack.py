...
if (len(sys.argv) < 3):
    ...
    imN[im1.shape[2]:,0:im1.shape[2],:,:]=np.multiply((imC/255.0),np.reshape(np.tile(colourL[0],imC.size/3),imC.shape))
    imC=0
    print 'Colouring sagittal plane and adding to final image..'
    imN[0:im1.shape[2],im1.shape[2]:,:imS.shape[2],:]=np.multiply((imS/255.0),np.reshape(np.tile(colourL[0],imS.size/3),imS.shape))
    imS=0
    print 'Colouring transverse plane and adding to final image..'
    imN[im1.shape[2]:,im1.shape[2]:,:imT.shape[2],:]=np.multiply((imT/255.0),np.reshape(np.tile(colourL[0],imT.size/3),imT.shape))
    imT=0
