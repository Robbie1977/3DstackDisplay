...
if (len(sys.argv) < 3):
    ...
    Cthird=imC.size/3
    imN[Ishape[2]:,0:Ishape[2],:,:]=np.multiply((imC/255.0),np.reshape(np.tile(colourL[0],imC.size/3),Cshape))
    ...
    imN[0:Ishape[2],Ishape[2]:,:Sshape[2],:]=np.multiply((imS/255.0),np.reshape(np.tile(colourL[0],imS.size/3),Sshape))
    ...
    imN[Ishape[2]:,Ishape[2]:,:Tshape[2],:]=np.multiply((imT/255.0),np.reshape(np.tile(colourL[0],imT.size/3),Tshape))
    ...
    for i in range(3,len(sys.argv)):
        ...
        if (Tsize <> im1.size):
            ...
            imC=np.array(np.reshape(np.repeat(np.transpose(im1,(0,2,1)),3),Cshape))
            print 'Colouring coronal plane and adding to final image..'
            imN[Ishape[2]:,0:Ishape[2],:,:]=np.add(np.uint16(np.multiply((imC/255.0),np.reshape(np.tile(colourL[(i-1)],imC.size/3),Cshape))),imN[Ishape[2]:,0:Ishape[2],:,:])
            imC=None
            gc.collect()
            ...
            imT=np.array(np.repeat(np.reshape(np.repeat(im1[:,:,::-1],3),[Tshape[0],Tshape[1],Ishape[2],Tshape[3]]),np.int(np.floor(float(Ishape[1])/float(Ishape[2]))),2))
            print 'Colouring transverse plane and adding to final image..'
            imN[Ishape[2]:,Ishape[2]:,:Tshape[2],:]=np.add(np.uint16(np.multiply((imT/255.0),np.reshape(np.tile(colourL[(i-1)],imT.size/3),Tshape))),imN[Ishape[2]:,Ishape[2]:,:Tshape[2],:])
            imT=None
            gc.collect()
            ...
            imS=np.array(np.repeat(np.reshape(np.repeat(np.transpose(im1,(2,1,0)),3),[Sshape[0],Sshape[1],Ishape[0],Sshape[3]]),np.int(np.floor(float(Ishape[1])/float(Ishape[0]))),2))
            print 'Colouring sagittal plane and adding to final image..'
            imN[0:Ishape[2],Ishape[2]:,:Sshape[2],:]=np.add(np.uint16(np.multiply((imS/255.0),np.reshape(np.tile(colourL[(i-1)],imS.size/3),Sshape))),imN[0:Ishape[2],Ishape[2]:,:Sshape[2],:])
            imS=None
            gc.collect()
    ...
    nrrd.write(str(sys.argv[i]).replace('.nrrd','_R.nrrd'),imN[:,:,:,0])
    print 'Saving green image..'
    nrrd.write(str(sys.argv[i]).replace('.nrrd','_G.nrrd'),imN[:,:,:,1])
    print 'Saving blue image..'
    nrrd.write(str(sys.argv[i]).replace('.nrrd','_B.nrrd'),imN[:,:,:,2])
